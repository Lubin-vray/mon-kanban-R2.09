...
const STATUS_LABELS = {
  ...
  in_progress: { label: '⚙️ En cours',   color: '#3B82F6' },
  review:      { label: '👀 Validation', color: '#F59E0B' },
  done:        { label: '✅ Terminée',   color: '#16A34A' },
};

export default function TaskCard({ task, onDelete, session }) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.75rem', alignItems: 'center' }}>
        ...
        <button
          ...
          style={{ fontSize: '0.8rem', color: '#1A8C82', background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginLeft: 'auto' }}
        >
          💬 {showComments ? 'Masquer' : 'Commentaires'}
        </button>
      </div>

      {/* ✅ TaskComments HORS du div des badges */}
